...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**Sensor Inputs Required**")
    st.caption("🔵 Optical · 🟣 SAR · 🔴 Thermal IR")
    st.divider()
    st.caption("Satellite Disaster Assessment System v0.2.0")
...
with tab_analyze:
    ...
    if results and st.session_state.get("analyzed_key") == current_key:
        ...
        badge_icon  = {"low": "🟢", "moderate": "🟡", "high": "🟠", "critical": "🔴"}.get(severity, "🔴")
        ...
        <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:12px;">
            <div>
                ...
                <span style="font-size:0.9em; color:#aaaaaa; margin-left:10px;">
                    Type: <b>{disaster_type}</b> &nbsp;|&nbsp; Sensor: {image_type}
                </span>
            </div>
        ...
        st.markdown("##### Multi-Spectral Band Metrics")
        ...
        st.caption(f"Mean Radiant Intensity: {band_stats.get('mean_intensity', 0.0):.1f}")

# ═══════════════════════ TAB 2 : ANALYSIS HISTORY ════════════════════════════
with tab_history:
    st.subheader("📜 Historical Analysis Reports")
    st.write("Browse all satellite ensemble analyses saved this session.")
    ...
    if not ok_hist or not history_items:
        st.info("No past reports. Analyze satellite images in the **Real-Time Analysis** tab to start logging.")
    else:
        ...
        for item in history_items:
            ...
            badge_icon  = {"low": "🟢", "moderate": "🟡", "high": "🟠", "critical": "🔴"}.get(sev, "🔴")
            ...
            with st.expander(
                ...
                <div class="history-card">
                    <div style="display:flex; flex-wrap:wrap; gap:10px; align-items:center; margin-bottom:10px;">
                        ...
                        <span>Sensor: <b>{pred.get('image_type_detected', 'Multi-Modal')}</b></span>
                    </div>
                    <div style="font-size:0.85em; color:#888; margin-bottom:8px;">
                        <b>Optical:</b> {images_meta.get('optical', '—')}<br>
                        <b>SAR:</b> {images_meta.get('sar', '—')}<br>
                        <b>Thermal IR:</b> {images_meta.get('thermal_ir', '—')}
                    </div>
